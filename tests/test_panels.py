...
async def test_sidebar_connected_lamp():
    ctx = await _ctx_with_sites(
        {"id": "x-com", "name": "X", "url": "https://x.com", "status": "connected"},
    )
    node = await panels.sidebar(ctx)
    assert "#22c55e" in str(node)  # green lamp hex in Html content


async def test_sidebar_error_lamp():
    ctx = await _ctx_with_sites(
        {"id": "x-com", "name": "X", "url": "https://x.com", "status": "error"},
    )
    node = await panels.sidebar(ctx)
    assert "#ef4444" in str(node)  # red lamp hex in Html content
